...
#define DRIVER_VERSION          "3.9.1 20180817"
...
struct stk3x1x_priv {
	...
	uint16_t ps_high_boot_cali;
	uint16_t ps_low_boot_cali;
#endif  
	uint16_t boot_cali;
#ifdef CTTRACKING
	bool ps_thd_update;
...
void stk_als_ir_get_corr(struct stk3x1x_priv *obj, u32 als_data)
{
	...
	if(obj->ir_code)
	{
		obj->als_correct_factor = 1400; //732
...
#endif	
//    printk("%s: stk test 1 >>>als_rawdata= %d\n", __func__, als_data);
    als_data_u32 = als_data;
	als_data_u32 = (als_data_u32 * obj->als_correct_factor) / 1000;
//	printk("%s: stk test 2 >>>als_reportdata= %d,factor = %d\n", __func__, als_data_u32,obj->als_correct_factor);
...
static int stk3x1x_enable_ps_set_thd(struct stk3x1x_priv *obj)
{
	...
	else if(obj->boot_cali == 1 && obj->ps_low_thd_boot < 5000 )
	{
		atomic_set(&obj->ps_high_thd_val, obj->ps_high_boot_cali); 
		atomic_set(&obj->ps_low_thd_val,  obj->ps_low_boot_cali);
		printk("use boot cali for initial \n");
	}
	/*
	else
	{
...
#else						
	obj->ps_high_thd_boot = obj->ps_stat_data[1] + 3*obj->stk_ht_n_ct;
	obj->ps_low_thd_boot  = obj->ps_stat_data[1] + 3*obj->stk_lt_n_ct;
	obj->ps_high_boot_cali = obj->ps_high_thd_boot;
	obj->ps_low_boot_cali = obj->ps_low_thd_boot;
	if(obj->ps_low_boot_cali > 100)
	{
		obj->stk_ht_n_ct = 50;
		obj->stk_lt_n_ct = 30;
	}
...
static int stk_ps_tune_zero_init(struct stk3x1x_priv *obj)
{
	...
	obj->ps_high_boot_cali = obj->hw->ps_threshold_high;
	obj->ps_low_boot_cali  = obj->hw->ps_threshold_low;
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	uint16_t ct_value = 0;
#endif
	int32_t word_data,true_ct = 0;
	u8 flag;
	bool ps_enabled = false;
	u8 buf[2],psctl_reg,ledctl_reg,flag_reg,state_reg;
	int ret, diff;
	...
	if (atomic_read(&stk3x1x_obj->ps_suspend))
        {
        APS_LOG("I2C suspend ,return the ps work \n");
        return 0;
        }

	ret = stk3x1x_master_recv(obj->client, 0x00, &state_reg, 1);
	if(ret < 0)
    {
	    APS_ERR( "%s read state_ctl reg fail, err=0x%x", __func__, ret);
	    return ret;	   
	}
	...
	if(atomic_read(&obj->psctrl_val) != psctl_reg || ledctl_reg != obj->ledctrl_val 
	                                              || atomic_read(&obj->state_val) != state_reg)
	{
			
	APS_LOG("stk reg check run state=0x%x, psctl=0x%x,ledctl =0x%x,\n",state_reg,psctl_reg,ledctl_reg);
	...
	if((obj->psi_set!=0))
	{
		if(obj->ps_distance_last == 1)
		{
			...
			if(ret == 0)
			{
				...
				word_data = (buf[0] << 8) | buf[1];
				if( small_data_count++ > 5000)
					small_data_count = 10;
				
				if(small_data_count%10 == 0)
				printk("stk ct tracking word data = %d\n",word_data);
			
				if(word_data > 0&& (small_data_count%15 == 0))
				{
					ct_value = atomic_read(&obj->ps_high_thd_val) - stk3x1x_obj->stk_ht_n_ct;
					true_ct  = ((obj->psi_set+3*obj->stk_ht_n_ct)>obj->ps_high_boot_cali&& obj->boot_cali)?
					           (obj->ps_high_boot_cali- 3*obj->stk_ht_n_ct ):obj->psi_set;
					printk("stk ct tracking true ct = %d \n",true_ct);
					ret = stk3x1x_read_flag(obj->client, &flag);
					if((word_data < ct_value) && ((ct_value - word_data) > 2) && ((flag&STK_FLG_PSINT_MASK)==0) 
				        && (obj->ps_distance_last == 1) && (obj->ps_thd_update == false)
					    &&word_data >true_ct)                                                                 
					{
						...
						if((atomic_read(&obj->ps_high_thd_val) + 100) < (obj->ps_high_boot_cali))
						{
							obj->ps_high_boot_cali = word_data + 3*obj->stk_ht_n_ct ;
							obj->ps_low_boot_cali = word_data + 2*obj->stk_ht_n_ct + obj->stk_lt_n_ct;
							obj->boot_cali = 1;
	...
	if(diff > obj->stk_max_min_diff)
	{
		...
		else
		{
			...
			{
				obj->ps_high_thd_boot = obj->psi + obj->stk_ht_n_ct + obj->stk_lt_n_ct;
				obj->ps_low_thd_boot = obj->psi + obj->stk_lt_n_ct + obj->stk_lt_n_ct;
				obj->boot_cali = 1;
				APS_LOG("%s: update boot HT=%d, LT=%d\n", __func__, obj->ps_high_thd_boot, obj->ps_low_thd_boot);
			}
		}
	...
	if(((1<<STK_BIT_PS) &  obj->pending_intr) && (obj->hw->polling_mode_ps == 0))
	{
		...
		if(near_far_state == 0){
	       ...
	       if(disable_flag)
	       {
	        ...
	        if((err = stk3x1x_read_ps(obj->client, &obj->ps)))
		    {
			  APS_ERR("stk3x1x read ps data when near by: %d\n", err);
			  goto err_i2c_rw;
		    }
			 APS_ERR("stk3x1x near by: psi =%d ,ps read =%d\n",obj->psi, obj->ps);
			 if(obj->ps > (obj->psi + STK_H_PS) && obj->psi > 100 ) {
		     ...
		     if((err = stk3x1x_read_ps(obj->client, &obj->ps)))
		     {
			 APS_ERR("stk3x1x read ps data when far away: %d\n", err);
			 goto err_i2c_rw;
		     } 
			 /*
...
static int STK3X1X_ReadChipInfo(struct i2c_client *client, char *buf, int bufsize)
{
...
static ssize_t chipinfo_show(struct device_driver *ddri, char *buf)
{
	...
	char strbuf[STK3X1X_BUF_SIZE] = {0};

	APS_LOG("fwq show_chipinfo_value\n");
	if (client == NULL) {
		APS_ERR("i2c client is null!!\n");
		return 0;
	}

	STK3X1X_ReadChipInfo(client, strbuf, STK3X1X_BUF_SIZE);
...
/*----------------------------------------------------------------------------*/
//tct need chipinfo which disaplay in mmi^M
...
static int create_chipinfo_node(void)
{
       ...
       if (err){
               APS_ERR("Failed to create device file(%s)!\n", dev_attr_psensor.attr.name);
               return -ENODEV;
       }
       return err;
}
...
static int als_get_data(int* value, int* status)
{
    ...
    if (atomic_read(&stk3x1x_obj->als_suspend))
       {
        APS_LOG("I2C suspend ,return the als read data\n");
        return 0;
       }
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    {
        APS_ERR("get stk3x1x dts node fail\n");
        err = -EFAULT;
		goto exit;
    }
...
//	obj->hw = get_cust_alsps_hw();
	...
	{
		APS_ERR("create chipinfo node %d\n", err);
	}
	/*
...
//	wakeup_source_init(&ps_lock,"ps_wakelock");
	if(1 == obj->hw->polling_mode_ps)
	{
