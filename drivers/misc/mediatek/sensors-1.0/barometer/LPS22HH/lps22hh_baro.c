...
int lps22hh_baro_init(struct lps22hh_baro *baro_obj, int reset_cali)
{
	...
	//set to low noise mode start
    struct lps22hh_data *obj = container_of(baro_obj, struct lps22hh_data, lps22hh_baro_data);
	struct i2c_client *client = obj->client;
	
    u8 databuf=0;

	printk("lps22hh_baro_init enter\n");
	
    databuf = 0x3E;
    res = lps22hh_i2c_write_block(client, LPS22HH_REG_CTRL_REG1, &databuf, 0x1);
    if (res < 0)
    {
		ST_ERR("write reg_ctl_reg1 register to 0x3E err!\n");
        return res;
    }	

   	databuf = 0x12;
    res = lps22hh_i2c_write_block(client, LPS22HH_REG_CTRL_REG2, &databuf, 0x1);
    if (res < 0)
    {
		ST_ERR("write reg_ctl_reg2 register to 0x12 err!\n");
        return res;
    }

   	databuf = 0;
	res = lps22hh_i2c_read_block(client, LPS22HH_REG_CTRL_REG1, &databuf, 0x01);
    if (res)
    {
		ST_ERR("read 0x10 register err!\n");
		return LPS22HH_ERR_I2C;
    }
	
   	databuf = 0;
	res = lps22hh_i2c_read_block(client, LPS22HH_REG_CTRL_REG2, &databuf, 0x01);
    if (res)
    {
		ST_ERR("read 0x11 register err!\n");
		return LPS22HH_ERR_I2C;
    }
	
	//set to low noise mode end

    baro_obj->odr = 0;
    res = lps22hh_baro_set_odr(baro_obj, LPS22HH_ODR_10HZ);	//power down
    if (res < 0)
    {
...
static int lps22hh_baro_read_data(struct lps22hh_baro *baro_obj, u8 *buf, int bufsize)
{
    ...
    { 
        ...
        baro = baro + baro_obj->cali_sw;

	res=lps22hh_i2c_read_block(client, LPS22HH_REG_CTRL_REG1, &data1, 0x01);
     if (res)
     {
         return snprintf(buf, PAGE_SIZE, "read chip 0x10 error!\n");
     }

	lps22hh_i2c_read_block(client, LPS22HH_REG_CTRL_REG2, &data2, 0x01);
     if (res)
     {
         return snprintf(buf, PAGE_SIZE, "read chip 0x11 error!\n");
     }
	lps22hh_i2c_read_block(client, LPS22HH_REG_CTRL_REG3, &data3, 0x01);
     if (res)
     {
         return snprintf(buf, PAGE_SIZE, "read chip 0x12 error!\n");
     }

	printk("tct barometer reg data,0x10:%x,0x11:%x,0x12:%x\n",data1,data2,data3);
