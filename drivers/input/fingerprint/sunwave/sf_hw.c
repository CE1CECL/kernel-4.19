...
#if (SF_POWER_MODE_SEL == PWR_MODE_GPIO)
    FINGER_POWER_OFF, FINGER_POWER_ON,
#endif
#if SF_SPI_DTS_CS
    FINGER_CS_SET,
#endif
...
static int sf_ctl_device_reset(void)
{
    ...
    xprintk(KERN_INFO, "Cydio %s(pinctrl) reset.\n", __FUNCTION__);
...
static int sf_ctl_device_power_by_gpio(bool on)
{
    ...
    if (on) {
		xprintk(KERN_INFO, "%s(gpio) pinctrl power on select\n", __FUNCTION__);
...
/*begin modified by zhikui.li,Turn off sw fingerprint power*/
    sf_ctl_device_power(false);
/*end modified by zhikui.li,Turn off sw fingerprint power*/
#if SF_MTK_CPU
    if (sf_pinctrl) {
...
static int sf_ctl_device_init_gpio_pins(struct sf_ctl_device *ctl_dev)
{
    ...
    printk("Cydio %s sf_pinctrl get begin\n",__func__);
    ...
    xprintk(KERN_INFO, "irq number is %d.\n", ctl_dev->irq_num);
    {
	    ...
	    xprintk(KERN_ERR, "Cydio find '%s' pinctrl_state.\n",sf_pinctrl_state_names[i]);
	    ...
	    printk("Cydio %s pinctrl_select_state(%s) ok \n",__func__, sf_pinctrl_state_names[i]);
        }
    }
...
int sf_platform_init(struct sf_ctl_device *ctl_dev)
{
    ...
    if (ctl_dev) {
	printk("%s will set gpio all",__func__);
