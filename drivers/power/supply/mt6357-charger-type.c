...
struct mtk_charger_type {
	...
	enum power_supply_type last_charge_type; /* Added by bitao.xiong for defect-10053764 on 2020-10-22 */
...
static int get_charger_type(struct mtk_charger_type *info)
{
	...
	dump_charger_name(info->charge_type);
...
void do_charger_detect(struct mtk_charger_type *info, bool en)
{
	...
	if (en) {
		...
		info->last_charge_type = POWER_SUPPLY_TYPE_UNKNOWN;
	}
...
irqreturn_t chrdet_int_handler(int irq, void *data)
{
	...
	if (info->charge_type == POWER_SUPPLY_TYPE_USB_FLOAT && 
		info->last_charge_type != info->charge_type) {
		info->last_charge_type = info->charge_type;
		cancel_delayed_work(&info->chr_work);
		schedule_delayed_work(&info->chr_work, msecs_to_jiffies(2000));
	} else {
		info->last_charge_type = info->charge_type;
	}
	/* End added by bitao.xiong for defect-10053764 on 2020-10-22 */
	return IRQ_HANDLED;
}
